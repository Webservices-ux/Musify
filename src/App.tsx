...
export default function App() {
  ...
  useEffect(() => {
    if (currentSong && wavesurfer.current) {
      ...
      wavesurfer.current.load(currentSong.url);
      ...
      <main className="flex-grow flex flex-col bg-gradient-to-b from-zinc-900 to-black overflow-y-auto relative">
        ...
        <div className="p-8 pb-32">
          <div className="flex flex-col md:flex-row items-end space-y-6 md:space-y-0 md:space-x-6 mb-8">
            <div className="w-48 h-48 md:w-60 md:h-60 shadow-2xl rounded-lg overflow-hidden flex-shrink-0 bg-zinc-800 flex items-center justify-center relative group">
              {currentSong ? (
                <>
                  <img 
        ...
        <div className="flex flex-col items-center space-y-2 w-1/2 max-w-2xl">
          ...
          <div className="flex items-center space-x-4 w-full px-4">
            ...
            <div className="flex-grow relative h-10 flex items-center">
              ...
              {!isWaveformReady && currentSong && (
                <div className="absolute inset-0 flex items-center justify-center bg-black/20">
                  <div className="flex space-x-1">
                    {[1, 2, 3].map(i => (
                      <div key={i} className="w-1 h-4 bg-emerald-500/50 animate-pulse rounded-full" style={{ animationDelay: `${i * 0.2}s` }} />
                    ))}
                  </div>
                </div>
              )}
